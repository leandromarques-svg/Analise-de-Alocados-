...
interface HeaderProps {
  ...
  onRefresh: () => void;
  onExportCSV: () => void;
  dataSource: "live" | "cache" | "stale_cache" | "fallback";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold tracking-tight text-slate-800 font-['Barlow']">
                  Análise de Alocados
                </h1>
              </div>
              <p className="text-xs text-slate-500">
                Gestão contínua de colaboradores
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-start lg:justify-end">
              ...
              <div className="flex items-center gap-2 bg-[#faf6fd] border border-[#e8d8f5] px-3 py-1.5 rounded-xl text-xs font-semibold text-[#401669]">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span
                      className={`px-1.5 py-0.2 text-[9px] font-black rounded-md uppercase ${
                        currentUser.role === "Administrador"
                          ? "bg-[#401669] text-white"
                  ...
                  {currentUser.role === "Cliente" && (
                    <div className="flex items-center gap-1 mt-0.5">
                      <span
                        className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[8.5px] font-extrabold bg-blue-100 text-blue-800 border border-blue-200"
                        title="Contatos diretos de colaboradores protegidos conforme LGPD (Lei nº 13.709/2018)"
                      >
                        <Shield className="w-2.5 h-2.5 text-blue-700" /> LGPD
                        Ativa
                      </span>
                    </div>
                  )}
                  {currentUser.role === "Comercial" &&
                    currentUser.clientesAtribuidos?.length && (
                      <p className="text-[10px] text-[#78549e] truncate max-w-[140px]">
                        Carteira: {currentUser.clientesAtribuidos.length}{" "}
                        clientes
                      </p>
                    )}
                </div>
            ...
            {currentUser?.role === "Administrador" && onOpenUsersModal && (
              <button
                onClick={onOpenUsersModal}
                className="bg-purple-100 hover:bg-purple-200 text-[#401669] font-bold text-xs px-3 py-1.5 rounded-xl transition-all flex items-center gap-1.5 border border-purple-200 cursor-pointer shadow-2xs"
                title="Gerenciar Usuários e Permissões"
              >
                <UserCog className="w-4 h-4 text-[#9c3aff]" />
            ...
            <div className="bg-slate-100 border border-slate-200/80 px-2.5 py-1.5 rounded-xl  text-[11px] flex items-center gap-2 font-medium text-slate-600">
              <span
                className={`w-2 h-2 rounded-full ${isLoading ? "bg-amber-500" : "bg-emerald-500 animate-pulse"}`}
              />
            ...
            <div className="relative group/bi">
              ...
              >
                <RefreshCw
                  className={`w-3.5 h-3.5 ${isLoading || isBackgroundUpdating ? "animate-spin" : ""}`}
                />
                <span className="hidden sm:inline">Atualizar</span>
                <Info
                  className="w-3.5 h-3.5 text-[#78549e]"
                  aria-hidden="true"
                />
              </button>
              <div
              ...
              >
                <div className="rounded-2xl border border-[#e8d8f5] bg-white p-4 text-left shadow-xl shadow-[#401669]/10">
                  ...
                  <p className="mt-2 text-xs leading-relaxed text-slate-600">
                    O BI coleta, organiza e entrega esses dados com agilidade. A
                    leitura sai direto da tabela de funcionários do GI Info, sem
                    planilha no caminho.
                  </p>
                  <p className="mt-3 text-xs font-semibold text-slate-700">
                    Seis vezes ao dia, a cada 2 horas. O dado fica disponível no
                    horário fechado.
                  </p>
            ...
            <button
              onClick={onExportCSV}
              className="bg-[#401669] hover:bg-[#521c87] text-white font-semibold text-xs px-3 py-1.5 rounded-xl transition-all shadow-xs flex items-center gap-1.5 active:scale-95 cursor-pointer"
